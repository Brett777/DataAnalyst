...
def executeInteractiveFeatureEngineeringCode(prompt, df, system_prompt = AI_SYSTEM_PROMPT):
    '''
        Executes the Python Code generated by the LLM
    '''
    print("Generating feature engineering code...")
    pythonCode = createFeatureEngineeringCodeGemini(prompt, system_prompt)
    pythonCode = extract_python_code(pythonCode)
    print(pythonCode)
                ...
                if startButton:
                    with st.spinner("Engineering Features... "):
                        ...
                        while attempts < max_retries:
                            ...
                            try:
                                if attempts == 0:
                                    #createFeatureEngineeringCodeGemini
                ...
                if startButton:
                    with st.spinner("Engineering Features... "):
                        ...
                        while attempts < max_retries:
                            ...
                            try:
                                if attempts == 0:
                                    pythonCode, results = executeInteractiveFeatureEngineeringCode(user_prompt, df, system_prompt)
                                else:
                                    pythonCode, results = executeFeatureEngineeringCodeReattempt(
                                        "Function to fix: \n" + str(pythonCode) + "\nERROR MESSAGE: \n" + str(results) + "\nORIGINAL USER PROMPT: \n" + user_prompt, 
                                        df, 
                                        "Fix the function based on error message and the users expected behavior")
